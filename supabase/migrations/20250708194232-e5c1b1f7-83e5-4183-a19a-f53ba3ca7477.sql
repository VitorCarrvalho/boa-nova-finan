-- Atualizar função approve_user para enviar email de aprovação
...
BEGIN
  ...
  IF _user_email IS NOT NULL THEN
    BEGIN
      ...
      SELECT content::jsonb INTO _email_response
      FROM http((
        'POST',
        'https://jryifbcsifodvocshnvuo.supabase.co/functions/v1/send-approval-email',
        ARRAY[
          http_header('Content-Type', 'application/json'),
          http_header('Authorization', 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImpyeWlmYmNzaWZvZHZvY3NobnVvIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTAxNzAwOTYsImV4cCI6MjA2NTc0NjA5Nn0.19evtStOEzVAbXX-Z27xi35JnTC7vd046EDRFrb9ETE')
        ],
        'application/json',
        _email_payload::text
      ));
      ...
      RAISE NOTICE 'Approval email sent to %: %', _user_email, _email_response;
...
-- Criar extensão http se não existir (necessária para chamar edge functions)
