...
-- 3. CRIAR PERFIL ASSISTENTE (se não existir)
...
WHERE NOT EXISTS (SELECT 1 FROM public.access_profiles WHERE name = 'Assistente');

-- 4. CRIAR PERFIL DIRETOR (se não existir)
INSERT INTO public.access_profiles (name, description, is_active, permissions)
SELECT 
  'Diretor',
...
WHERE NOT EXISTS (SELECT 1 FROM public.access_profiles WHERE name = 'Diretor');
