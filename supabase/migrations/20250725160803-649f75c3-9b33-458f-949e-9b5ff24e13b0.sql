-- Update existing profiles with fixed variable names
DO $$
DECLARE
    _congregation_id uuid;
    _gerente_profile_id uuid;
    _diretor_profile_id uuid;
    _presidente_profile_id uuid;
BEGIN
    -- Get necessary IDs
    SELECT id INTO _congregation_id FROM congregations WHERE name = 'IPTM - Mesquisa' LIMIT 1;
    SELECT id INTO _gerente_profile_id FROM access_profiles WHERE name = 'Gerente Financeiro' LIMIT 1;
    SELECT id INTO _diretor_profile_id FROM access_profiles WHERE name = 'Diretor' LIMIT 1;
    SELECT id INTO _presidente_profile_id FROM access_profiles WHERE name = 'Presidente' LIMIT 1;
    ...
    SET 
        ...
        approval_status = 'ativo',
        profile_id = _diretor_profile_id,
        congregation_id = _congregation_id,
        approved_at = now()
    ...
    SET 
        ...
        approval_status = 'ativo', 
        profile_id = _presidente_profile_id,
        congregation_id = _congregation_id,
        approved_at = now()
