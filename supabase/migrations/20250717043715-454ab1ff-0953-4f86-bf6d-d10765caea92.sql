-- Corrigir bug no cadastro de novos usuários - Versão corrigida
...
-- 4. Verificar se o perfil padrão "Membro" existe (usando WHERE NOT EXISTS)
INSERT INTO public.access_profiles (name, description, permissions, is_active)
SELECT 'Membro', 'Perfil padrão para membros da congregação', '{}', true
WHERE NOT EXISTS (
  SELECT 1 FROM public.access_profiles WHERE name = 'Membro'
);
